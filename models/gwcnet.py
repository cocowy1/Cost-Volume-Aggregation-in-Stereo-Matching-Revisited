...
class GwcNet(nn.Module):
    ...
    def vis_tsne(self, cost, disp_true_down):
        # scale to 1/8 res.
        cost = F.adaptive_avg_pool3d(cost[:, :, :, 2:, :], (48//2, 134//2, 240//2))
        B, C, D, H, W = cost.shape
        disparity_planes = disp_true_down.max()
        
        volume = cost.new_zeros([B, C, H, W])
        for disp_id in range(disparity_planes+1):
            mask = (disp_true_down == disp_id)
            if mask.sum() == 0: continue
            feats = cost[:,:,disp_id]*mask
            volume += feats

        return volume

    def vis_tsne1(self, out):
        # scale to 1/8 res.
        cost = self.classif2(out)
        cost = F.adaptive_avg_pool3d(cost[:, :, :, 2:, :], (48//2, 134//2, 240//2))
        return cost.squeeze(1)
    def forward(self, left, right, disp_true_down):
        # left, right = inputs[0,:3,...].unsqueeze(0), inputs[0,3:,...].unsqueeze(0)
        ...
        if self.training:
            ...
            volume = self.vis_tsne1(out2)
            # cost3 = self.classif3(out3)
            # # cost3 = F.upsample(cost3, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear')
            # cost3 = torch.squeeze(cost3, 1)
            # pred3 = F.softmax(cost3, dim=1)
            
            # pred3_ = disparity_regression(pred3, self.maxdisp)
            # print('full training time = %.3f seconds' % ((time.time() - start_time)))
            return volume
