...
class GwcNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        # final = F.upsample(pred2, scale_factor=(4, 4), mode='bilinear')

        out1 = self.classif1(out1)
        # cost2 = F.upsample(out2, scale_factor=(4, 4, 4), mode='trilinear')
        out1 = torch.squeeze(out1, 1)
        cost1 = F.softmax(out1, dim=1)
        pred1 = disparity_regression(cost1, self.maxdisp // 4)
        pred1 = self.prop(guidance['g'], pred1)


        if self.training:
            out0 = self.classif0(cost0)
            # out0 = F.upsample(out0, scale_factor=(2, 2, 2), mode='trilinear')
            out0 = torch.squeeze(out0, 1)
            pred0 = F.softmax(out0, dim=1)
            # pred0 = disparity_regression(pred0, self.maxdisp)

            out_dca0 = F.upsample(prob_volume1, scale_factor=(8, 8, 8), mode='trilinear')
            out_dca0 = torch.squeeze(out_dca0, 1)
            pred_dca0 = F.softmax(out_dca0, dim=1)
            pred_dca0 = disparity_regression(pred_dca0, self.maxdisp)
            return [pred0.squeeze(1)], [pred_dca0.squeeze(1), pred1.squeeze(1)]

        else:
            return pred1.squeeze(1), prob_volume1.squeeze(1)
