...
class GwcNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        cost0 = torch.squeeze(out0, 1)
        cost0 = F.softmax(cost0, dim=1)
        pred0 = disparity_regression(cost0, self.maxdisp // 4)
        pred0 = self.prop(guidance['g'], pred0)

        # bu
        # out0 = self.classif0(cost0)
        # cost0 = F.upsample(out0, scale_factor=(4, 4, 4), mode='trilinear')
        # cost0 = torch.squeeze(cost0, 1)
        # cost0 = F.softmax(cost0, dim=1)
        # pred0 = disparity_regression(cost0, self.maxdisp)


        if self.training:
            return [out0.squeeze(1)], [pred0.squeeze(1)]
        else:
            return pred0.squeeze(1), out0.squeeze(1)
